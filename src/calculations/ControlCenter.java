...
//        LIVE_CALCULATING
    ...
    public void createDump(String name, int seconds, int accuracy, int stepDuration, List<Planet> planets) {
        ...
        for (int i = 1; i < seconds; i++) {
            if (i % accuracy == 0) {
                processNextSecond(planets, true);
            } else {
                processNextSecond(planets, false);
            }
...
//        System.out.println();
    ...
    private void processNextSecond(List<Planet> planets, boolean applyForce) {
        if (applyForce) {
            for (int i = 0; i < planets.size(); i++) {
                planets.get(i).setAcceleration(new Vector3d());
            }

            for (int i = 0; i < planets.size(); i++) {
                for (int j = i + 1; j < planets.size(); j++) {
                    gravity(planets.get(i), planets.get(j), true, true, false);
                }
            }
        }
    ...
    private double gravity(Planet a, Planet b, boolean applyToA, boolean applyToB, boolean returnForce) {
        double G = 6.67e-11d;
...
//        }
    ...
    public List<Planet> initRandomObjects() {
    ...
    public Planet generateRandomObject(double maxPosition, double maxSpeed) {
        Random r = new Random();
...
//        Vector3d vel = new Vector3d(r.nextDouble() * maxSpeed, r.nextDouble() * maxSpeed, r.nextDouble() * maxSpeed);
        ...
        Planet planet = new Planet("p" + name, pos, vel, new Vector3d(), mass, 6786e3d / 2.0d);
        return planet;
    }
    ...
    private Planet initSun() {
        //Planet Sun = new Planet("Sun", new Vector(), new Vector(), new Vector(), 332946 * 5.976e24d, 696e6d);
        Planet Sun = new Planet("Sun", new Vector3d(), new Vector3d(), new Vector3d(), 1.989e30d, 696e6d);
        return Sun;
    }

    private Planet initMercury() {
        Planet Mercury = new Planet("Mercury", new Vector3d(-5.79e10d, 0.0d, 0.0d), new Vector3d(0.0d, 0.0d, -4.787e4d), new Vector3d(), 5.976e24d * 0.055d, 4878e3d / 2.0d);
        return Mercury;
    }

    private Planet initVenus() {
        Planet Venus = new Planet("Venus", new Vector3d(0.0d, 0.0d, -1.082e11d), new Vector3d(3.503e4d, 0.0d, 0.0d), new Vector3d(), 5.976e24d * 0.815d, 12103e3d / 2.0d);
        return Venus;
    }

    private Planet initEarth() {
        Planet Earth = new Planet("Earth", new Vector3d(1.496e11d, 0.0d, 0.0d), new Vector3d(0.0d, 0.0d, 2.979e4d), new Vector3d(), 5.976e24d, 6378e3d);
        //Planet Earth = new Planet("Earth", new Vector(1.496e11d, 0.0d, 0.0d), new Vector(0.0d, 0.0d, 0.0d), new Vector(), 5.976e24d, 6378e3d);
        return Earth;
    }

    private Planet initMars() {
        Planet Mars = new Planet("Mars", new Vector3d(0.0d, 0.0d, 2.279e11d), new Vector3d(-2.41e4d, 0.0d, 0.0d), new Vector3d(), 5.976e24d * 0.108d, 6786e3d / 2.0d);
        return Mars;
    }

    private Planet initJupiter() {
        Planet Jupiter = new Planet("Jupiter", new Vector3d(-7.783e11d, 0.0d, 0.0d), new Vector3d(0.0d, 0.0d, -1.31e4d), new Vector3d(), 5.976e24d * 317.9d, 143.0e6 / 2.0d);
        return Jupiter;
    }

    private Planet initSaturn() {
        Planet Saturn = new Planet("Saturn", new Vector3d(0.0d, 0.0d, -1.427e12d), new Vector3d(9.6e3d, 0.0d, 0.0d), new Vector3d(), 5.976e24d * 95.181d, 120.0e6d / 2.0d);
        return Saturn;
    }

    private Planet initUranus() {
        Planet Uranus = new Planet("Uranus", new Vector3d(2.8966e12d, 0.0d, 0.0d), new Vector3d(0.0d, 0.0d, 6.8e3d), new Vector3d(), 5.976e24d * 14.531d, 51118.0e3d / 2.0d);
        return Uranus;
    }

    private Planet initNeptune() {
        ...
        Planet Neptune = new Planet("Neptune", new Vector3d(0.0d, 0.0d, 4.4966e12d), new Vector3d(-5.4e3d, 0.0d, 0.0d), new Vector3d(), 102.4e24d, 49528.0e3d / 2.0d);
        return Neptune;
    }
